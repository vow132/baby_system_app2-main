...
onShow(async () => {
  try {
    ...
    if (res.code === 0 && res.data) {
      settings.notification = res.data.channel_app ?? hasRequestedSubscribe()
      settings.sms = res.data.channel_sms ?? false
      settings.quietHours = res.data.quiet_hours ?? ''
    }
  } catch { /* 接口未就绪时保持默认值 */ }
  // 后端显示开启但本地未订阅 → 弹微信授权
  if (settings.notification && !hasRequestedSubscribe()) {
    const tmplIds = [SUBSCRIBE_TEMPLATES.ALERT, SUBSCRIBE_TEMPLATES.REMINDER, SUBSCRIBE_TEMPLATES.REPORT].filter(Boolean)
    if (tmplIds.length > 0) {
      const accepted = await requestSubscribe(tmplIds)
      if (!accepted) {
        settings.notification = false
        await updatePushSettings({ channel_app: false }).catch(() => {})
      }
    }
  }
})
...
async function savePushSettings() {
  try {
    ...
    const res = await updatePushSettings({
      ...
      quiet_hours: settings.quietHours || undefined,
    })
  ...
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        ...
        const subscribeCache = uni.getStorageSync(SUBSCRIBED_KEY)
        ...
        if (subscribeCache) uni.setStorageSync(SUBSCRIBED_KEY, subscribeCache)
